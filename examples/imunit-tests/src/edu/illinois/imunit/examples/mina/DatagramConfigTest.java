 ...
 *
 */
package edu.illinois.imunit.examples.mina;
...
public class DatagramConfigTest {
    ...
    @Test
    @Schedule(name = "testMessageReceive", value = "afterReceive@recThread->test@main")
...
//          Original code:
            //OPWAIT            for (int i = 0; i < 30; i++) {
            //OPWAIT               if (result.length() == 2) {
            //OPWAIT     break;
            //OPWAIT    }
            //OPWAIT Thread.sleep(100);
            //OPWAIT            }
            
            //Thread.sleep(3000); //INS-SLEEP
            fireEvent("test");
            assertEquals("FH", result);
            assertEquals("testMessageReceive","FH", result);
        } finally {
            acceptor.unbind();
        }
    }
    ...
    private class MockFilter extends IoFilterAdapter {
        ...
        public void messageReceived(NextFilter nextFilter, IoSession session,
            ...
            Thread.currentThread().setName("recThread");
            result += "F";
